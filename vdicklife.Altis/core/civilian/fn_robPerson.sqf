...
if (CASH > 0) then {
    ...
    if (life_HC_isActive) then {
        [getPlayerUID _robber,_robber getVariable ["realname",name _robber],"52FA"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
    } else {
        [getPlayerUID _robber,_robber getVariable ["realname",name _robber],"52FA"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
    };
