            ...
            if (count _sPos > 1 && {_distance < 15}) then {
                _text = switch (true) do {
                    ...
                    case (side _x isEqualTo west && {!isNil {_x getVariable "rank"}}): {format ["<img image='%1' size='1'></img> %2",switch ((_x getVariable "rank")) do {
                        ...
                        case 6: {"icons\ico_cop_ranks\sergeantFC.paa"};
                        case 7: {"icons\ico_cop_ranks\lieutenant.paa"};
                        case 8: {"icons\ico_cop_ranks\captain.paa"};
                        case 9: {"icons\ico_cop_ranks\Acheif.paa"};
                        case 10: {"icons\ico_cop_ranks\deputy_chief.paa"};
                        case 11: {"icons\ico_cop_ranks\chief.paa"};
